...
# 显示启动提示
zenity --info --title="VLLM服务启动器" --text="即将启动Qwen3和Qwen2.5-VL服务\n\n请等待启动完成提示..." --width=300

# 启动第一个服务
zenity --notification --text="开始启动Qwen3-14B-FP8服务..."
...
# 启动第二个服务
zenity --notification --text="开始启动Qwen2.5-VL-7B服务..."
...
if [ $RESULT1 -eq 0 ]; then
    RESULT_TEXT+="✅ Qwen3-14B-FP8服务启动成功 (端口: 8003)\n"
else
    if [ $RESULT1 -eq 1 ]; then
        RESULT_TEXT+="❌ Qwen3-14B-FP8服务启动失败，进程已退出\n"
    else
        RESULT_TEXT+="⌛ Qwen3-14B-FP8服务启动超时\n"
    fi
    RESULT_TEXT+="📄 查看日志: vllm_qwen3.log\n"
fi

if [ $RESULT2 -eq 0 ]; then
    RESULT_TEXT+="✅ Qwen2.5-VL-7B服务启动成功 (端口: 8002)\n"
else
    if [ $RESULT2 -eq 1 ]; then
        RESULT_TEXT+="❌ Qwen2.5-VL-7B服务启动失败，进程已退出\n"
    else
        RESULT_TEXT+="⌛ Qwen2.5-VL-7B服务启动超时\n"
    fi
